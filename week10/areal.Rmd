...
Works well for non-stationary cases:

$$Y_i = \beta_0(i) + \beta_1(i)x_{1} + \ldots + \beta_p(i)x_{p} $$
with the estimator:

$\beta(i)=(X^TW(i)X)^{-1}X^TW(i)Y$, where $W(i)$ is a matrix of weights spefici to $i$-th location such that observations nearer to this location are given greater weight nearer to $i$ are given greater weight than observations further away. Typically, $W$ can be otained from a weighting (or kernel) function. 
...
# Applied to GLM (generalized linear model) case
...
# GWR local coefficient estimates for the exposure to TCE site covariate
